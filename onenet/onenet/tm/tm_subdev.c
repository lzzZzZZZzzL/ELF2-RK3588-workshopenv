...
static int32_t subdev_data_callback(const uint8_t* name, void* data, uint32_t data_len)
{
    ...
    if (NULL != osl_strstr(name, TM_TOPIC_SUBDEV_TOPO_CHANGE)) {
        ...
        if (ERR_OK == ret) {
            tm_send_response(TM_TOPIC_SUBDEV_TOPO_CHANGE_REPLY, id, 200, 0, NULL, 0, SDK_REQUEST_TIMEOUT);
        } else {
            tm_send_response(TM_TOPIC_SUBDEV_TOPO_CHANGE_REPLY, id, 100, 0, NULL, 0, SDK_REQUEST_TIMEOUT);
        }
        ...
        if (ERR_OK == ret) {
            tm_send_response(TM_TOPIC_SUBDEV_TOPO_GET_REPLY_RESULT, id, 200, 0, NULL, 0, SDK_REQUEST_TIMEOUT);
        } else {
            tm_send_response(TM_TOPIC_SUBDEV_TOPO_GET_REPLY_RESULT, id, 100, 0, NULL, 0, SDK_REQUEST_TIMEOUT);
        }
        ...
        osl_free(input_data);
        if (ERR_OK == ret) {
            tm_send_response(TM_TOPIC_SUBDEV_SERVICE_INVOKE_REPLY, id, 200, 0, service_data, osl_strlen(output_data), SDK_REQUEST_TIMEOUT);
        } else {
            cJSON_Delete((cJSON*)service_data);
            tm_send_response(TM_TOPIC_SUBDEV_SERVICE_INVOKE_REPLY, id, 100, 0, NULL, 0, SDK_REQUEST_TIMEOUT);
        }
        if (output_data) {
        ...
        osl_free(prop_list);
        if (ERR_OK == ret) {
            tm_send_response(TM_TOPIC_SUBDEV_PROP_GET_REPLY, id, 200, 1, prop_data, osl_strlen(prop_data), SDK_REQUEST_TIMEOUT);
        } else {
            tm_send_response(TM_TOPIC_SUBDEV_PROP_GET_REPLY, id, 100, 0, NULL, 0, SDK_REQUEST_TIMEOUT);
        }
        if (prop_data) {
            osl_free(prop_data);
        }
        ...
        osl_free(prop_data);
        if (ERR_OK == ret) {
            tm_send_response(TM_TOPIC_SUBDEV_PROP_SET_REPLY, id, 200, 1, NULL, 0, SDK_REQUEST_TIMEOUT);
        } else {
            tm_send_response(TM_TOPIC_SUBDEV_PROP_GET_REPLY, id, 100, 0, NULL, 0, SDK_REQUEST_TIMEOUT);
        }
    }
...
static int32_t subdev_topo_add_delete(const uint8_t* action, const uint8_t* product_id, const uint8_t* dev_name, const uint8_t* access_key, uint32_t timeout_ms)
{
    ...
    dev_token_generate(dev_token, SIG_METHOD_SHA1, 1924833600, product_id, dev_name, access_key);
